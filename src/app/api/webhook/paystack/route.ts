...
async function handleChargeSuccess(data: any) {
  ...
  if (agentCommissionKobo > 0 && transaction.agentId) {
    ...
    // For sale transactions, mark commission as held; release tasks are handled by close/release endpoints.
    if (String(metadata?.type).toLowerCase() === 'sale' || transaction.type === 'sale') {
      await prisma.transaction.update({
        where: { id: transaction.id },
        data: { status: 'commission_held', agentCommissionStatus: 'held', commissionHoldReason: 'Awaiting deal close verification / dispute window' },
      });
    }
  }
