  ...
  return (
    <DashboardShell navigation={ADMIN_NAVIGATION}>
      ...
      <div className="space-y-6">
        ...
        <p className="text-zinc-500">Resolve transaction disputes.</p>
        {disputes.length === 0 ? (
                ...
                {disputes.map((tx) => (
                  <tr key={tx.id} className="border-b">
                    ...
                    <td className="p-3 text-right">
                      <Link href={`/dashboard/admin/disputes/${tx.id}`} className="px-3 py-1.5 text-sm bg-emerald-500 text-on-primary rounded-lg">Review</Link>
                    </td>
                  </tr>
                ))}
              </tbody>
