    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage supported countries, jurisdictions, and feature availability</p>
        </div>
        <button
      ...
      {showAdd && (
        <div className="rounded-lg border border-border bg-card p-6">
          ...
          <form onSubmit={addCountry} className="grid grid-cols-2 gap-4">
            <input placeholder="Code (e.g. GH)" value={form.code} onChange={e => setForm({...form, code: e.target.value})} className="rounded-lg border border-border bg-background px-3 py-2" />
            <input placeholder="Name (e.g. Ghana)" value={form.name} onChange={e => setForm({...form, name: e.target.value})} className="rounded-lg border border-border bg-background px-3 py-2" />
            <input placeholder="Currency (e.g. GHS)" value={form.currency} onChange={e => setForm({...form, currency: e.target.value})} className="rounded-lg border border-border bg-background px-3 py-2" />
            <input placeholder="Locale (e.g. en-GH)" value={form.locale} onChange={e => setForm({...form, locale: e.target.value})} className="rounded-lg border border-border bg-background px-3 py-2" />
            <input placeholder="Timezone (e.g. Africa/Accra)" value={form.timezone} onChange={e => setForm({...form, timezone: e.target.value})} className="rounded-lg border border-border bg-background px-3 py-2" />
            <div className="flex gap-2">
              <button type="submit" className="rounded-lg bg-primary px-4 py-2 text-primary-foreground">Save</button>
              <button type="button" onClick={() => setShowAdd(false)} className="rounded-lg border border-border px-4 py-2">Cancel</button>
            </div>
          </form>
        </div>
      )}
      ...
      <div className="space-y-4">
        {countries.map(country => (
          <div key={country.code} className="rounded-lg border border-border bg-card p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2">
                <span className={`text-xs px-2 py-1 rounded ${country.active ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-600"}`}>
                  {country.active ? "Active" : "Inactive"}
                </span>
                <button onClick={() => toggleActive(country.code, country.active)} className="text-sm text-primary hover:underline">
              ...
              <div className="mt-4 border-t border-border pt-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {country.capabilities.map(cap => (
                    <span key={cap.id} className={`text-xs px-2 py-1 rounded ${cap.enabled ? "bg-green-100 text-green-700" : cap.available ? "bg-amber-100 text-amber-700" : "bg-gray-100 text-gray-600"}`}>
