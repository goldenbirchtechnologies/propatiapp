      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-xs font-label-md uppercase tracking-wider" className="text-neutral-400", marginTop: 'mt-1' }}>Schedule and track property inspections</p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="card p-4"><p className="text-xs font-label-md uppercase tracking-wider" className="text-neutral-400" }}>Total</p><p className="text-2xl font-bold" className="text-white" }}>{initialInspections.length}</p></div>
          <div className="card p-4"><p className="text-xs font-label-md uppercase tracking-wider" className="text-neutral-400" }}>Scheduled</p><p className="text-2xl font-bold text-info">{initialInspections.filter((i) => i.status === 'scheduled').length}</p></div>
          <div className="card p-4"><p className="text-xs font-label-md uppercase tracking-wider" className="text-neutral-400" }}>Completed</p><p className="text-2xl font-bold text-[#00ff66]">{initialInspections.filter((i) => i.status === 'completed').length}</p></div>
          <div className="card p-4"><p className="text-xs font-label-md uppercase tracking-wider" className="text-neutral-400" }}>Cancelled</p><p className="text-2xl font-bold text-red-500">{initialInspections.filter((i) => i.status === 'cancelled').length}</p></div>
        </div>
        ...
        <div className="card overflow-hidden">
          ...
          {filtered.length === 0 ? (
            <div className="card-body text-center py-16"><Phone className="w-16 h-16 mx-auto mb-4" className="text-neutral-400", opacity: 0.5 }} /><h3 className="font-headline-sm font-bold text-lg mb-2" className="text-white" }}>No inspections</h3><p className="text-xs font-label-md uppercase tracking-wider" className="text-neutral-400" }}>Schedule your first inspection.</p></div>
          ) : (
            <table className="w-full divide-y divide-[#262626]">
              <thead><tr className="border-b" style={{ borderColor: 'border-[#262626]' }}>
                <th className="text-left p-4 text-sm font-label-md uppercase tracking-wider" className="text-neutral-400" }}>Property</th>
                ...
                <th className="text-left p-4 text-sm font-label-md uppercase tracking-wider" className="text-neutral-400" }}>Report</th>
              </tr></thead>
              <tbody>
                {filtered.map((i) => {
                  ...
                  return (
                    <tr key={i.id} className="border-b transition-colors hover:bg-[#171717]/30" style={{ borderColor: 'border-[#262626]' }}>
                      <td className="p-4 text-sm font-medium" className="text-white" }}>{i.listing?.title || '—'}</td>
                      <td className="p-4 text-xs font-label-md uppercase tracking-wider" className="text-white" }}>{new Date(i.scheduledAt).toLocaleDateString('en-NG', { day: '2-digit', month: 'short', year: 'numeric' })}</td>
                      ...
                      <td className="p-4 text-xs font-label-md uppercase tracking-wider" className="text-neutral-400" }}>{i.reportUrl ? 'Available' : 'Pending'}</td>
                    </tr>
                  );
                })}
              </tbody>
