  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-xs font-label-md uppercase tracking-wider text-neutral-400 mt-1">Client reviews and ratings</p>
      </div>

      <div className="card p-6">
        <div className="flex items-center gap-4">
          <div className="text-headline-sm font-bold text-white">{avgRating.toFixed(1)}</div>
          ...
          <span className="text-xs font-label-md uppercase tracking-wider text-neutral-400">{initialReviews.length} reviews</span>
        </div>
      </div>
          ...
          {initialReviews.map((r) => (
            <div key={r.id} className="card p-6">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-xs font-label-md uppercase tracking-wider text-neutral-400">{new Date(r.date).toLocaleDateString('en-NG', { day: '2-digit', month: 'short', year: 'numeric' })}</span>
              </div>
              ...
              <p className="text-xs font-label-md uppercase tracking-wider text-neutral-400">{r.comment}</p>
            </div>
          ))}
        </div>
      )}
