  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-xs font-label-md uppercase tracking-wider" style={{ color: 'text-on-surface-variant', marginTop: 'mt-1' }}>Upload thousands of properties via CSV or Excel</p>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <div className="card p-6">
            <h2 className="font-headline-sm font-bold mb-4" style={{ color: 'text-primary' }}>Upload File</h2>
            <div className="border-2 border-dashed rounded-xl p-10 text-center" style={{ borderColor: 'border-outline-variant' }}>
              <Upload className="w-12 h-12 mx-auto mb-4" style={{ color: 'text-on-surface-variant' }} />
              <p className="text-sm font-medium" style={{ color: 'text-primary' }}>Click to upload or drag and drop</p>
              <p className="text-xs font-label-md uppercase tracking-wider mt-1" style={{ color: 'text-on-surface-variant' }}>CSV, XLSX or XLS (max. 50MB)</p>
            </div>
          </div>
          ...
          <div className="card p-6">
            ...
            {status === 'idle' ? (
              <p className="text-xs font-label-md uppercase tracking-wider" style={{ color: 'text-on-surface-variant' }}>No active import. Upload a file to begin.</p>
            ) : status === 'processing' ? (
              <div className="space-y-3">
                ...
                <p className="text-xs font-label-md uppercase tracking-wider" style={{ color: 'text-on-surface-variant' }}>Processing 3,240 of 5,412 rows...</p>
              </div>
            ) : status === 'done' ? (
              <div className="space-y-2">
                ...
                <p className="text-xs font-label-md uppercase tracking-wider" style={{ color: 'text-on-surface-variant' }}>5,412 properties loaded. 0 errors.</p>
              </div>
            ) : (
        ...
        <div className="space-y-6">
          <div className="card p-6">
            ...
            <p className="text-xs font-label-md uppercase tracking-wider mb-4" style={{ color: 'text-on-surface-variant' }}>Use the official template to avoid column mapping errors.</p>
          ...
          <div className="card p-6">
            ...
            <ul className="space-y-3 text-xs font-label-md uppercase tracking-wider" style={{ color: 'text-on-surface-variant' }}>
