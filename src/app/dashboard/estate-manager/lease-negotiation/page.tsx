    ...
    return (
      <DashboardShell navigation={ESTATE_MANAGER_NAVIGATION} userRole="estate_manager" userName="Estate Manager" userAvatar={undefined}>
        <div className="space-y-6">
          ...
          <p className="text-xs font-label-md uppercase tracking-wider" style={{ color: 'text-on-surface-variant' }}>Unable to load negotiation terms.</p>
      ...
      <div className="space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-3">
              ...
              <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold border border-outline-variant bg-warning/10 text-warning">Active Negotiation</span>
            </div>
        ...
        <div className="rounded-xl border border-outline-variant shadow-sm overflow-hidden bg-surface-container-lowest">
          <div className="p-5 border-b border-outline-variant bg-surface-container-low">
            ...
            <p className="text-xs font-label-md uppercase tracking-wider mt-1" style={{ color: 'text-on-surface-variant' }}>Original listing vs proposed counter-offer</p>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead className="bg-surface-container-high border-b border-outline-variant">
                <tr>
                  <th className="px-5 py-4 text-xs font-label-md uppercase tracking-wider" style={{ color: 'text-on-surface-variant' }}>Term</th>
                  <th className="px-5 py-4 text-xs font-label-md uppercase tracking-wider" style={{ color: 'text-on-surface-variant' }}>Original Listing</th>
              ...
              <tbody className="divide-y divide-outline-variant">
                {terms.map((item) => (
                  <tr key={item.term} className="hover:bg-surface-container-low transition-colors">
                    ...
                    <td className="px-5 py-4 text-xs font-label-md uppercase tracking-wider" style={{ color: 'text-on-surface-variant' }}>{item.original}</td>
              ...
              <div key={idx} className="flex items-center justify-between p-3 bg-surface-container-low rounded-lg border border-outline-variant">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-label-md uppercase tracking-wider" style={{ color: 'text-on-surface-variant' }}>{doc.date} · {doc.size}</p>
                  </div>
                </div>
