  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="font-headline-sm font-bold text-white">Revenue Signature</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="glass-card p-4 border-white/[0.08] bg-zinc-950/50">
          <p className="text-xs uppercase tracking-wider text-zinc-500">Total Revenue</p>
          <p className="text-2xl font-bold text-[#00ff66] font-mono">₦{(totalRevenue / 1e6).toFixed(2)}M</p>
        </div>
        <div className="glass-card p-4 border-white/[0.08] bg-zinc-950/50">
          <p className="text-xs uppercase tracking-wider text-zinc-500">Service Charges</p>
          <p className="text-2xl font-bold text-zinc-300 font-mono">₦{(totalServiceCharges / 1e6).toFixed(2)}M</p>
        </div>
        <div className="glass-card p-4 border-white/[0.08] bg-zinc-950/50">
          <p className="text-xs uppercase tracking-wider text-zinc-500">Rent Transactions</p>
          <p className="text-2xl font-bold text-white font-mono">₦{(totalRent / 1e6).toFixed(2)}M</p>
        </div>
        <div className="glass-card p-4 border-white/[0.08] bg-zinc-950/50">
          <p className="text-xs uppercase tracking-wider text-zinc-500">Net Signature</p>
          <p className={`text-2xl font-bold font-mono ${totalNet >= 0 ? 'text-[#00ff66]' : 'text-red-500'}`}>
            {totalNet >= 0 ? '+' : ''}₦{(totalNet / 1e6).toFixed(2)}M
          </p>
        </div>
        <div className="glass-card p-4 border-white/[0.08] bg-zinc-950/50">
          <p className="text-xs uppercase tracking-wider text-zinc-500">Avg Transaction</p>
          <p className="text-2xl font-bold text-white font-mono">₦{(avgTransactionAmount / 1000).toFixed(0)}K</p>
          <p className="text-xs text-zinc-500 mt-1">{transactionCount} transactions</p>
        </div>
        <div className="glass-card p-4 border-white/[0.08] bg-zinc-950/50">
          <p className="text-xs uppercase tracking-wider text-zinc-500">Pending Settlement</p>
          <p className="text-2xl font-bold text-red-500 font-mono">₦{(pendingSettlement / 1e6).toFixed(2)}M</p>
          <p className="text-xs text-zinc-500 mt-1">Awaiting clearance</p>
        </div>
      </div>
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-white/[0.08] hover:bg-transparent">
                ...
                <TableHead className="text-right text-zinc-500">Total Revenue</TableHead>
                <TableHead className="text-right text-zinc-500">Service Charges</TableHead>
                <TableHead className="text-right text-zinc-500">Rent Transactions</TableHead>
            ...
            <TableBody>
              {filtered.length === 0 ? (
                ...
                filtered.map((m) => (
                  <TableRow key={m.month} className="border-white/[0.08] transition-colors hover:bg-zinc-900est">
                    ...
                    <TableCell className="text-right text-zinc-300 font-mono text-sm">₦{m.serviceCharges.toLocaleString()}</TableCell>
