                  ...
                  payouts.map((w) => (
                    <tr key={w.id} className="hover:bg-obsidian-800-lowest transition-colors">
                      ...
                      <td className="px-5 py-4">
                        <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${w.status === 'released' ? 'bg-success/10 text-[#00ff66] border border-success/20' : w.status === 'pending' ? 'bg-warning/10 text-warning border border-warning/20' : 'bg-muted/30 text-muted-foreground'}`}>
                          {w.status}
                        </span>
                      </td>
                    </tr>
