  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-zinc-400" style={{ marginTop: 'var(--space-vs)' }}>Submit and track maintenance requests</p>
      </div>
      ...
      {initialTickets.length === 0 ? (
        <div className="card-body text-center py-16">
          ...
          <p  className="text-zinc-400">Maintenance requests will appear here.</p>
        </div>
      ) : (
        <div className="card overflow-hidden">
          <table className="w-full">
            <thead><tr className="border-b border-zinc-800">
              ...
              <th className="text-left p-4 text-sm font-medium text-zinc-400">Submitted</th>
            </tr></thead>
            <tbody>
              {initialTickets.map((t) => (
                <tr key={t.id} className="border-b transition-colors hover:bg-muted/30 border-zinc-800">
                  <td className="p-4 font-medium text-sm text-white">{t.title}</td>
                  <td className="p-4 text-sm capitalize text-white">{t.priority}</td>
                  <td className="p-4 text-sm capitalize text-zinc-400">{t.status}</td>
                  <td className="p-4 text-sm text-zinc-400">{new Date(t.createdAt).toLocaleDateString('en-NG', { day: '2-digit', month: 'short', year: 'numeric' })}</td>
                </tr>
              ))}
            </tbody>
          </table>
