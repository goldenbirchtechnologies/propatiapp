      ...
      {steps.map((s) => (
        ...
        >
          <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${
            s.num === currentStep ? 'bg-primary text-white' : s.num < currentStep ? 'bg-teal-500 text-white' : 'bg-muted text-muted-foreground border border-border'
          }`}>
          ...
          <span className={`text-sm font-semibold ${s.num === currentStep ? 'text-primary' : 'text-muted-foreground'}`}>{s.label}</span>
        </div>
      ))}
    </div>
  );
          ...
          <Card className="hidden lg:block">
            <CardContent className="p-6">
              <h3 className="font-heading text-base font-semibold text-primary mb-3 flex items-center gap-2">
                <AlertCircle className="w-4 h-4 text-orange-500" /> Emergency Protocol
              </h3>
        ...
        <div className="lg:col-span-8">
          ...
          {step === 1 && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
              {[
                { icon: '✓', color: 'bg-teal-500 text-teal-700', title: 'Certified Techs', desc: 'All our technicians are background-checked and certified.' },
                { icon: '⏱', color: 'bg-muted text-primary', title: 'Fast Response', desc: 'Requests are usually acknowledged within 2 hours.' },
                { icon: '📋', color: 'bg-orange-500/10 text-orange-600', title: 'Track Progress', desc: 'Real-time tracking of your technician\'s arrival time.' },
              ].map((tip, i) => (
                <Card key={i}>
